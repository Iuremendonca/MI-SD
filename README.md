...
A estratégia adotada combinou simulação em múltiplos níveis com comparação sistemática contra um golden model em Python.

```
┌─────────────────┐     vetores .mem      ┌─────────────────┐
│  Golden Model   │──────────────────────▶│   Testbench     │
│  (Python/NumPy) │                       │   (elm_tb.v)    │
│  pred_ref[K]    │◀── comparação ────────│   pred_hw[K]    │
└─────────────────┘                       └─────────────────┘
         │                                        │
         └──────────────────────────────┬─────────┘
                                        ▼
...
|------|-----------|-----------|
| TC-01 | Validação individual do MAC (aritmética Q4.12) | ✅ Passou |
| TC-02 | Validação do sigmoid piecewise (4 segmentos) | ✅ Passou |
| TC-03 | Validação do argmax (10 entradas sequenciais) | ✅ Passou |
| TC-04 | Sincronização da camada oculta (h_saida × referência Python) | ✅ Passou |
| TC-05 | Sincronização da camada de saída (y_saida × referência Python) | ✅ Passou |
| TC-06 | Saturação do MAC (overflow e underflow) | ✅ Passou |
| TC-07 | Reset durante processamento (FSM → REPOUSO) | ✅ Passou |
| TC-08 | Inferência completa — K vetores MNIST | ✅ Passou (após correção β) |
| TC-09 | Dois START consecutivos sem reset | ✅ Passou |
| TC-10 | Validação na placa DE1-SoC | ✅ Passou |

### 11.4 Automação

```bash
# Script completo: gera vetores → simula → verifica → gera relatório
